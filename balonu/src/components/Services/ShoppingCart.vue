        ...
        <div class="shopping-list">
            <div v-for="item in filteredItems" :key="item.id" class="list-item">
                <div class="list-item-content">
                    ...
                    <br>
                    <span class="item-price">{{ (item.quantite_produit * item.prix_produit).toFixed(2) }}€</span>
                </div>
            ...
            <router-link :to="{ name: 'checkout', params: { items: filteredItems } }">
...
export default {
  props: {
    ...
    computed: {
        ...
        totalPrice() {
            const total = this.filteredItems.reduce((acc, item) => acc + (item.prix_produit * item.quantite_produit), 0);
