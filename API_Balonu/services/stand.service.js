...
async function getStandsByIdUtilisateur(id) {
    const client = await pool.connect();
    try {
...
async function createStand(libelle_stand, description_stand, image_stand, id_emplacement, id_categorie_stand) {
    const client = await pool.connect();
    try {
...
async function deleteStand(id) {
    ...
    try {
        ...
        const result = await client.query('DELETE FROM stand WHERE id_stand = $1 RETURNING *', [id]);
